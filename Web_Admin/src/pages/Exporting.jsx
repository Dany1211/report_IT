...
export default function Exporting() {
  ...
  const filteredReports = reports.filter((r) => {
    return (
      ...
      (priorityFilter === "All" || r.remarks === priorityFilter) &&
  ...
  const handleExportCSV = () => {
    const dataToExport = selectedReports.length > 0 ? selectedReports : filteredReports;
    const headers = ["ID", "Reporter", "Email", "Issue", "Description", "Location", "Priority", "Status", "Created At"];
    const csvRows = [
      headers.join(","),
      ...dataToExport.map((r) =>
        [r.id, r.reporter_name, r.reporter_email, r.issue_type, r.description, r.location, r.remarks, r.status, r.created_at].join(",")
      ),
    ];
    const csvString = csvRows.join("\n");
    const blob = new Blob([csvString], { type: "text/csv" });
  ...
  const handleExportPDF = () => {
    ...
    dataToExport.forEach((r, i) => {
      doc.text(
        `${r.id}. ${r.issue_type} [${r.location}] - ${r.remarks} (${r.status})`,
        10,
        20 + i * 10
      );
    });
...
Write in a structured but plain-text format.`,
      ...
      const data = await response.json();
      let summary = data.choices[0].message.content;
      summary = summary.replace(/[#*]/g, ""); // clean up
      setAiReport(summary);
    } catch (err) {
      console.error("AI Report Error:", err);
      setAiReport("❌ Failed to generate AI report.");
  ...
  // ✅ AI Report PDF (multi-page support)
  const handleDownloadAIReport = () => {
    if (!aiReport) return;
  ...
  return (
    <div className="min-h-screen p-6 bg-[#FFF9F0]">
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        ...
        >
          <option value="All">All Priorities</option>
          {[...new Set(reports.map((r) => r.remarks))].map((p) => (
